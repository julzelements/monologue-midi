...
describe('Monologue Parameter Validation', () => {
  ...
  describe('JSON Schema Validation', () => {
    dumpFiles.forEach((dumpFile) => {
      it(`should validate ${dumpFile}_converted.json successfully`, () => {
        const filePath = join(__dirname, 'data', 'parsed', `${dumpFile}_converted.json`);
  ...
  describe('Value Ranges', () => {
    it('should have valid 10-bit values (0-1023) for parameters', () => {
      const filePath = join(__dirname, 'data', 'parsed', 'dump1_converted.json');
      const data = JSON.parse(readFileSync(filePath, 'utf8'));
